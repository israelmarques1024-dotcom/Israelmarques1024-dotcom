...
const PATCHED_SHA='12db9ea036ec9942e442c09f8983b1407ece966fab3bc47e4688470fb506144c';
const CACHE='doritus_ultra_bundle_v281';
...
try{
  ...
  if(patched!==PATCHED_SHA)throw new Error('Patch v2.8.1 não bate com SHA esperado: '+patched);
  GM_setValue(CACHE,source);
}catch(err){
  source=GM_getValue(CACHE,'');
  if(!source){console.error('[Doritus Ultra] Não foi possível carregar v2.8.1:',err);return;}
}
