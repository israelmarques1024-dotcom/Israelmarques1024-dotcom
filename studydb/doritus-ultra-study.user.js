// ==UserScript==
// @name         Doritus Ultra Study
// @namespace    doritus-ultra
// @version      2.9.1
// @description  Doritus Ultra Study v2.9.1 — bundle completo e independente das versões antigas.
// @author       israelmarques1024-dotcom
// @match        *://*/*
// @grant        GM_xmlhttpRequest
// @grant        GM_registerMenuCommand
// @grant        GM_setValue
// @grant        GM_getValue
// @connect      raw.githubusercontent.com
// @updateURL    https://raw.githubusercontent.com/israelmarques1024-dotcom/Israelmarques1024-dotcom/main/studydb/doritus-ultra-study.user.js
// @downloadURL  https://raw.githubusercontent.com/israelmarques1024-dotcom/Israelmarques1024-dotcom/main/studydb/doritus-ultra-study.user.js
// @run-at       document-idle
// ==/UserScript==

(async()=>{
'use strict';
const BASE='https://raw.githubusercontent.com/israelmarques1024-dotcom/Israelmarques1024-dotcom/main/studydb/ultra-v291/';
const PARTS=5;
const EXPECTED='8c29d522f1e1ed5b2e067ef19e37f10a6d366efcfa788a56280248dce7c82891';
const CACHE='doritus_ultra_bundle_v291_exact';

const get=url=>new Promise((resolve,reject)=>GM_xmlhttpRequest({
  method:'GET',url,
  onload:r=>r.status>=200&&r.status<300?resolve(r.responseText):reject(new Error('HTTP '+r.status)),
  onerror:()=>reject(new Error('Falha de rede'))
}));
const hash=async text=>{
  const d=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(text));
  return [...new Uint8Array(d)].map(x=>x.toString(16).padStart(2,'0')).join('');
};
const unpack=async b64=>{
  if(typeof DecompressionStream==='undefined')throw new Error('DecompressionStream indisponível');
  const bytes=Uint8Array.from(atob(b64),c=>c.charCodeAt(0));
  const stream=new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'));
  return await new Response(stream).text();
};

let source='';
try{
  const chunks=await Promise.all(Array.from({length:PARTS},(_,i)=>get(BASE+`part-${String(i).padStart(2,'0')}.txt`)));
  source=await unpack(chunks.join(''));
  const got=await hash(source);
  if(got!==EXPECTED)throw new Error('SHA-256 inesperado: '+got);
  GM_setValue(CACHE,source);
}catch(err){
  console.error('[Doritus 2.9.1] Falha ao carregar bundle:',err);
  source=GM_getValue(CACHE,'');
  if(!source)return;
}
try{eval(source)}catch(err){console.error('[Doritus 2.9.1] Falha ao iniciar:',err)}
})();
